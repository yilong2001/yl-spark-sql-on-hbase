...
object SlideWindow {
  def main(args: Array[String]) : Unit = {
    ...
    //val sQLContext = ssc.sparkContext

    val searchLogsDStream = ssc.socketTextStream("localhost", 9999)

    val searchWordsDStream = searchLogsDStream.map { searchLog => searchLog.split(" ")(0) }

    val searchWordPairDStream = searchWordsDStream.map { searchWord => (searchWord, 1) }

    val windowSearchWordPairDStream = searchWordPairDStream.window(Duration(10000),Duration(2000))
